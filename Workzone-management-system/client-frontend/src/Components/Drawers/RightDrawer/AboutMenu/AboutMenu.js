...
const AboutMenu = () => {
	...
	const onChangeHandler = function (e) {
		if (userRole !== 'admin') return; // Prevent non-admin edits
	...
	const handleSaveClick = () => {
		...
		boardDescriptionUpdate(workspaceId, board.id, description, dispatch);
	};
		...
		<Container>
			...
			<SectionContainer>
				...
				<DescriptionSectionContainer ref={descriptionAreaRef}>
					...
					/>
					{textareaFocus && userRole === 'admin' && ( // Show buttons only for admins
						<BottomButtonGroup
							title='Save'
