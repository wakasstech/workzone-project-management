...
const LabelsPopover = (props) => {
	...
	const handleCreateClick = async (text, color, backColor) => {
		...
		await labelCreate(workspaceId, thisCard.cardId, thisCard.listId, thisCard.boardId, text, color, backColor, dispatch);
	...
	const handleColorBoxClick = async (labelId, selected) => {
		await labelUpdateSelection(workspaceId, thisCard.cardId, thisCard.listId, thisCard.boardId, labelId, selected, dispatch);
	};

	const handleDeleteClick = async (labelId) => {
		props.arrowCallback(false);
		props.titleCallback('Labels');
		await labelDelete(workspaceId, thisCard.cardId, thisCard.listId, thisCard.boardId, labelId, dispatch);
	};
