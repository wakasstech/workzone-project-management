...
const listSlice = createSlice({
	...
	reducers: {
		...
		updateListMembers: (state, action) => {
			console.log('Redux Action payload:', action.payload);	  
			const { listId, members } = action.payload;
			console.log('Redux List ID:', listId);
			console.log('Redux Updated Members:', members);		  
			const list = state.allLists.find((list) => list._id === listId);
			console.log('REDUX LIST', list)
			if (list) {
			  list.members = members;
			  console.log('Redux List after update:', list);
			}		  
			console.log('Redux Updated State:', state);
			return state; // Make sure to return the updated state
		  },
		...
		addChecklistItemForCard: (state, action) => {
			const { listId, cardId, checklistId, _id, text, assignedTo } = action.payload;
			state.allLists = state.allLists.map((list) => {
				if (list._id === listId) {
					list.cards = list.cards.map((card) => {
						if (card._id === cardId) {
							card.checklists = card.checklists.map((list) => {
								if (list._id.toString() === checklistId.toString()) {
									list.items.push({ _id: _id, text: text,assignedTo:assignedTo, completed: false });
								}
								return list;
							});
						}
		...
		setChecklistItemTextOfCard: (state, action) => {
			const { listId, cardId, checklistId, checklistItemId, text, assignedTo } = action.payload;
			state.allLists = state.allLists.map((list) => {
				if (list._id === listId) {
					list.cards = list.cards.map((card) => {
						if (card._id === cardId) {
